...
const EXTENSION_VIRTUAL = "tc.template.virtual.tsx";
...
function makeVirtualUri(document: vscode.TextDocument): vscode.Uri {
	...
	return vscode.Uri.file(path.join(folder, `${baseName}.${EXTENSION_VIRTUAL}`));
}
...
async function hideVirtualFiles() {
	...
	excludes[`**/*.${EXTENSION_VIRTUAL}`] = false;
	...
	context.subscriptions.push(
		vscode.languages.onDidChangeDiagnostics((event) => {
			for (const uri of event.uris) {
				if (uri.fsPath.endsWith(`.${EXTENSION_VIRTUAL}`)) {
		...
		vscode.languages.registerCompletionItemProvider(
			{ language: "tsx-template" },
			{
				async provideCompletionItems(document, position) {
					...
					const completions =
						(await vscode.commands.executeCommand<vscode.CompletionList>("vscode.executeCompletionItemProvider", doc.uri, mapToVirtualPosition(position, virtualUri.startPosition))) ?? [];
							...
							if (item.range instanceof vscode.Range) {
								...
								item.range = {
									inserting: new vscode.Range(
										mapToTemplatePosition(item.range.inserting.start, virtualUri.startPosition),
										mapToTemplatePosition(item.range.inserting.end, virtualUri.startPosition)
									),
									replacing: new vscode.Range(
										mapToTemplatePosition(item.range.replacing.start, virtualUri.startPosition),
										mapToTemplatePosition(item.range.replacing.end, virtualUri.startPosition)
									),
								};
							}
						}
